...
Pattern pads **#1–#8** hold reusable variations; melodic and drummer pads turn green when they contain a note or hit with nonzero velocity and appear as **1–8** in arranger lanes and palettes. Red groups and violet supergroups use the same colours in sequencers and arranger lanes; rests stay dark. Assemble phrases in the upper-right workspace of melodic, drummer and controller sequencers and arpeggiators: drag pads, Cmd/Ctrl-click to group selections, right-click to split groups, and Apply shared phrase edits explicitly. Play loops the whole workspace and outlines the currently playing pad, phrase or rest. Deleting an unused phrase expands its temporary workspace references, preserving the assembly. Assemble song order in the multitrack arranger with pads, groups, supergroups and rests. Hold a speaker icon for 250 ms to preview a phrase: during arranger playback it enters at the next cycle boundary; while stopped it plays without moving the song cursor. Release resumes the previous playback. Live arpeggiator previews follow the configured rhythm cycle or master bar and use incoming or held notes. The highlighted Play or Stop button shows the arranger’s state. Right-click pads and phrases to save custom colours shared by their occurrences. Each sequencer can use its own meter, grid, and beat ratio around a shared tempo. Explore patterns of different lengths, odd meters, and rhythms that move against one another.
...
Lay out those patterns in the **multitrack arranger** to build an introduction, repeat a phrase, bring drums in, or leave space for a change of mood. Melodic, drummer, and controller tracks share one timeline, with looping and navigation controls for working on a passage. Play runs from the current cursor to the song end, including trailing rests. **Loop song**, immediately left of Rewind, repeats the whole song when enabled and is saved with the performance. Marked loop ranges repeat independently of this toggle. Repeating lanes stay within those song bounds. Choose **Arrangement** or **Manual pads** with the Playback source toggle in each sequencer. New devices start with Manual pads; placing their first item in an arranger lane selects Arrangement. Later source choices are saved with the performance. Start individual devices to develop a pattern with backing parts: Arrangement devices join the selected song passage, while Manual pads plays the pad selected for editing. While Manual pads runs, click another pad to queue it for the end of the current pattern: orange marks the queued pad and blue-green marks the playing pad. Manual pads keep playing through song loops, endings, and arranger Play/Stop. Compact track headers expand with a triangle to reveal a wrapping drag-and-drop palette that hides rest-only pattern pads. Drop at an element edge to insert, or beyond the lane’s end to add a gap automatically. Move items left or right into adjacent rests, including across their original position, to redistribute silence without shifting other items. Right-click to group, ungroup, create variations, or remove material. Mute or solo individual lanes—even when they share an instrument—without losing their place in the song. These temporary lane controls do not change the mixer or saved performance.

Select song sections across all lanes by dragging the arranger’s top beat ruler, or select some lanes by dragging a rectangle in empty timeline space (Shift-drag over existing elements). Right-click to copy, overwrite, or insert a section while shifting later material. **Cmd/Ctrl+Shift+D** repeats the selection immediately after itself and inserts time across all lanes: two sections A–B become A–A–B. Context menus also offer insertion on selected lanes only. **Alt/Option-drag** copies a range; add **Shift** to insert time across all lanes. The arranger’s Undo/Redo buttons share a 25-action history for timeline edits, groups, variations, colours and automatic source selection on first placement. Hold either button to select a history entry. History is saved with the performance; selection and clipboard survive view switches within the session. Copies reuse existing pads and phrases; use Create variation for independent changes. See [song-section editing](documentation/performance/multitrack_arranger.md#copy-and-insert-song-sections).
