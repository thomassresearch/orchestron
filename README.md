...
The filter catalog includes `atone` for audio signals, `atonek` for control signals, and `atonex` for cascaded high-pass filtering with audio-rate or control-rate cutoff modulation. All three include English/German/French/Spanish port help; see [high-pass filters](documentation/instrument_design/opcode_catalog_and_documentation.md#atone-high-pass-filters) for defaults and connections.
...
![Workflow and patch format](documentation/instrument_design/control_flow.md).
...
All six performance device types have editable names. Click the pen beside a device name, then Save/Enter or Cancel/Escape. New names must be nonempty, contain at most 65 Unicode characters, contain no HTML or angle brackets, and be unique across the performance (ignoring case and surrounding whitespace). Names are retained in saved performances and native JSON/ZIP exports; existing imported names are preserved.

The live performance area provides an instrument rack where instruments can be selected and assigned to MIDI channels. Each rack slot uses the same type-grouped, searchable patch picker as Instrument Design: expand an instrument type to browse alphabetically, or search names and descriptions across types after at least four characters and a 500 ms typing pause. Template patches remain excluded, and rack selection locks while instruments are running. A melodic sequencer can then drive those instruments, with scale and mode selection, note entry that supports choosing notes both in-scale and out-of-scale, and per-step chord choices including the standard `5` power-chord voicing (root + perfect fifth). Performances support up to 16 melodic, 16 drummer, and 16 controller sequencers. `tempoBPM` stays global, while every melodic sequencer, drummer sequencer, and controller sequencer now has its own meter (`2..7` over `4` or `8`), grid in steps per beat (`2`, `4`, or `8`), and beat ratio (`1:1`, `2:1`, `3:2`, `4:3`, `3:4`, `5:4`, `4:5`, `7:4`). That makes polymeter and true per-sequencer polyrhythm possible inside one performance.
...
Instrument Design has collapsible **Patch controls**, with instrument tabs always visible; Runtime starts hidden and opens through **Show runtime**. Perform device groups and the arranger collapse independently while keeping header actions available. A collapsed rack shows MIDI channel and patch name in one horizontally scrolling row, including every loaded instance. Panel choices survive view switches until browser reload and are not saved into performances. See [panel controls](documentation/performance/performance.md#collapsible-panels). Collapsed Perform bodies, including nested Mixer sections, suspend visual construction, animation and meter subscriptions while playback continues. Expanding restores drafts, selections, clipboard, zoom and scroll at the current playback position. Editor state stays in memory while Perform is open and resets on New, Load or Import. Collapsing closes temporary menus, cancels drags and releases manually held piano notes.

The instrument graph has horizontal and vertical scrollbars for navigating large patches, including when Patch controls is collapsed. Drag a scrollbar or focus it with Tab and use the arrow keys; scrolling stays synchronized with canvas pan, zoom, and Fit.
...
Both CSD modes include only rack instances assigned to melodic/drummer sequencers, piano-roll keyboards or arpeggiator output channels, plus continuous instruments referenced by mixer routes, Master or insert ownership. Device assignments count even when stopped or empty; continuous generators with implicit direct audio output are retained. Unused instances, their routes and their exclusive assets are omitted before compilation. Native performance bundles still preserve the complete rack. Each CSD starts with an Orchestron header containing the performance title and description, creation timestamp, project summary and GitHub link. Further comments include instrument names, descriptions, instance-to-Csound references, routing endpoints and ports, insert/Master roles, pre/post-fader taps and initial mixer settings.

Implementation details for always-on effect instruments are documented in [EFFECT_INSTRUMENTS.md](EFFECT_INSTRUMENTS.md).

The multitrack arranger combines melodic sequencers, drummer sequencers, and controller sequencers in one shared timeline. **Fit**, immediately left of **Zoom -**, adjusts zoom to show the full length of the longest track without horizontal scrolling and returns the view to the start (DE: **Einpassen**, FR: **Ajuster**, ES: **Ajustar**). Cassette-style transport controls start only sequencers with `Pad Looper` enabled and stop arranger-driven pad-loop sequencers without stopping manually started non-pad-loop sequencers or the rack instrument engine; `Play` also stops sequencers whose `Pad Looper` is off so arranger playback stays synchronized. Rewind/fast-forward move in shared `1-beat` transport blocks, double-clicking `Stop` resets to the selected loop start or step `0`, dragging on the bottom **Loop Range** ruler previews a range and releasing repeats it down to a single beat; clicking anywhere on that ruler clears the range and seeks to the clicked beat, continuing playback if already running and staying stopped otherwise, right-click menus can insert pads or existing group/super-group tokens and copy/paste selected phrase blocks into later gaps or the sequence end, and the section now exposes the same integrated `?` help flow as the other perform devices.
...
The [`perf_controller` virtual opcode](documentation/performance/performance_controllers.md) adds independent I-rate knobs to each rack instrument for ADSR times, distortion, and other performance settings. Five compact controls fit across a standard rack card before additional controls wrap. Controls support linear/logarithmic scales, precise entry, and reset to patch defaults. New notes use changed values; continuous instruments require a rack restart. Save Performance, native bundles, and both Csound exports preserve the settings.
