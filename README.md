...
**If / Switch and Drumset:** choose one synthesis case per note, with explicit case membership and one mono/stereo audio result. The drumset template maps 36/38/42 to kick/snare/hi-hat with overlapping decays. [Workflow and patch format (EN/DE/FR/ES)](documentation/instrument_design/control_flow.md).

**DE:** If/Switch wählen pro Note einen Synthesezweig; das Schlagzeug nutzt 36/38/42 mit unabhängigem Ausklingen. **FR :** If/Switch choisissent un cas par note ; la batterie utilise 36/38/42 avec des décroissances indépendantes. **ES:** If/Switch eligen un caso por nota; la batería usa 36/38/42 con decaimientos independientes.
