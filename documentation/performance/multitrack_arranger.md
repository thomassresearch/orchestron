...
**Play** runs from the current cursor to the end of the longest lane, including trailing rests, and then stops at that position. With a marked loop range, it repeats only that range. **Repeat track sequence** can repeat a shorter lane within these bounds; it never extends song playback beyond the end. The loop ruler and lane playheads share the same horizontal origin when zooming and scrolling.
