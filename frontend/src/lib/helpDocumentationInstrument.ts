...
- Hover over the description field to see the full description in a tooltip.
...
- Mit der Maus über das Beschreibungsfeld fahren, um die vollständige Beschreibung als Tooltip anzuzeigen.
...
- Survolez le champ de description pour voir la description complète dans une infobulle.
...
- Pasa el ratón sobre el campo de descripción para ver la descripción completa en una ayuda emergente.
...
const instrumentTypeHelp = {
  english: "### Types and patch search\n\nLoad Patch opens collapsed type groups with counts and alphabetically sorted patches. Type at least four characters to search names and descriptions across all groups; results update after a 500 ms typing pause. Shorter queries restore browsing. Up/Down navigates, Enter selects or expands, and Escape closes. Closing resets search. Already-open patches are excluded.\n\nTypes survive save, clone, templates, restored tabs and instrument/performance JSON/ZIP export and import. Older always-on patches become Continuous; other types are inferred from names, then descriptions, with Melody as the fallback. You can correct the type; renaming does not reclassify it. Audio effects that must run continuously belong to Continuous, while Effects / Noise uses MIDI notes.",
  german: "### Typen und Patch-Suche\n\nPatch laden öffnet eingeklappte Typgruppen mit Anzahl und alphabetisch sortierten Patches. Mindestens vier Zeichen durchsuchen Namen und Beschreibungen aller Gruppen; Ergebnisse erscheinen nach 500 ms Tipp-Pause. Kürzere Eingaben zeigen wieder die Gruppen. Auf/Ab navigiert, Enter wählt oder klappt auf, Escape schließt. Schließen setzt die Suche zurück. Bereits geöffnete Patches werden ausgeblendet.\n\nTypen bleiben beim Speichern, Klonen, bei Vorlagen, wiederhergestellten Tabs und Instrument-/Performance-Import und -Export als JSON/ZIP erhalten. Alte immer aktive Patches werden Kontinuierlich; andere Typen werden aus Namen, danach Beschreibungen abgeleitet, mit Melodie als Standard. Der Typ ist korrigierbar; Umbenennen ändert ihn nicht. Dauerhafte Audioeffekte gehören zu Kontinuierlich, Effekte / Geräusche verwendet MIDI-Noten.",
  french: "### Types et recherche de patchs\n\nCharger patch ouvre des groupes de types repliés avec leur nombre de patchs, triés alphabétiquement. Saisissez au moins quatre caractères pour rechercher dans les noms et descriptions de tous les groupes ; les résultats apparaissent après une pause de 500 ms. Une saisie plus courte rétablit les groupes. Haut/Bas navigue, Entrée sélectionne ou déplie, Échap ferme. Fermer réinitialise la recherche. Les patchs déjà ouverts sont exclus.\n\nLes types sont conservés lors de l’enregistrement, du clonage, des modèles, de la restauration des onglets et des imports/exports JSON/ZIP d’instruments ou de performances. Les anciens patchs toujours actifs deviennent Continu ; les autres types sont déduits des noms puis des descriptions, avec Mélodie par défaut. Vous pouvez corriger le type ; renommer ne le modifie pas. Les effets audio permanents utilisent Continu, tandis qu’Effets / Bruit utilise les notes MIDI.",
  spanish: "### Tipos y búsqueda de patches\n\nCargar patch abre grupos de tipos plegados con recuentos y patches ordenados alfabéticamente. Escribe al menos cuatro caracteres para buscar nombres y descripciones en todos los grupos; los resultados aparecen tras una pausa de 500 ms. Una consulta más corta restaura los grupos. Arriba/Abajo navega, Intro selecciona o expande y Escape cierra. Cerrar restablece la búsqueda. Se excluyen los patches ya abiertos.\n\nLos tipos se conservan al guardar, clonar, usar plantillas, restaurar pestañas e importar/exportar instrumentos o interpretaciones en JSON/ZIP. Los antiguos patches siempre activos pasan a Continuo; los demás tipos se deducen primero de los nombres y luego de las descripciones, con Melodía por defecto. Puedes corregir el tipo; renombrar no lo cambia. Los efectos de audio permanentes usan Continuo, mientras Efectos / Ruido usa notas MIDI."
};
for (const language of ["english", "german", "french", "spanish"] as const) {
  instrumentHelpDocuments.instrument_patch_toolbar[language].markdown += "\n\n" + instrumentTypeHelp[language];
}
