...
function MixerBody({ onStop }: { onStop: () => void }) {
  ...
  return <> {notice && <div className="my-2 rounded border border-amber-800 p-2 text-xs text-amber-100">{t("migration")} <button className={button} onClick={() => useAppStore.setState({ migrationNotice: false })}>{t("dismiss")}</button></div>}{(error || syncError) && <div role="alert" className="my-2 text-xs text-rose-300">{error || syncError}</div>}<RetainedScroll owner="mixer" field="stripsScroll" className="my-3 flex gap-2 overflow-x-auto pb-2">{bindings.filter((b) => b.id !== graph.masterId && !graph.insertOwners[b.id]).map((b) => strip(b.id))}{graph.masterId ? strip(graph.masterId) : <button className={`${button} h-fit shrink-0`} disabled={running} onClick={() => void useAppStore.getState().ensureMaster().catch((e: unknown) => setError(String(e)))}>{t("createMaster")}</button>}</RetainedScroll><div className="max-w-md text-xs">{t("audioOutput")}<Meter id="$output" name={t("audioOutput")} /></div>{running && <button className={button} onClick={onStop}>{t("stop")}</button>}<EditorDetails owner={"mixer"} field="routing" initiallyOpen className="mt-3 border-t border-slate-800 pt-2" summary={<>{t("routing")}</>}>{() => <><label className="my-2 flex items-center gap-2 text-xs">{t("master")}<select className={field} disabled={running} value={graph.masterId ?? ""} onChange={(e) => applyGraph({ ...graph, masterId: e.target.value || null })}><option value="">—</option>{graph.masterId && !byId.has(graph.masterId) && <option value={graph.masterId}>{name(graph.masterId)}</option>}{bindings.filter((b) => byId.get(b.id)?.has_direct_output).map((b) => <option key={b.id} value={b.id}>{name(b.id)}</option>)}</select></label><div className="my-2 flex flex-wrap items-end gap-2">
