import { EditorDetails, EditorScope, RetainedScroll, usePerformanceEditorState } from "./sequencer/PerformanceEditorState";
...
import { buildSequencerConfigSnapshot, normalizeSessionInstrumentAssignments } from "../store/appStoreModel";
import { useAppStore } from "../store/useAppStore";
import type { AudioDiagnostic, AudioGraph, AudioRoute, GuiLanguage } from "../types";

const field = "min-w-0 rounded border border-slate-600 bg-slate-950 px-2 py-1 text-xs text-slate-100";
const diagnosticKeys: Record<string, Parameters<ReturnType<typeof audioCopy>>[0]> = { missing_patch: "missing", missing_master: "missing", broken_route: "brokenRoute", feedback: "feedback", no_output_path: "noOutput", no_audio_source: "noInput", parallel_output: "parallelOutput", incomplete_stereo: "incompleteStereo", unconnected_input: "unusedInput", unconnected_output: "unusedOutput" };
...
function MixerBody({ onStop }: { onStop: () => void }) {
  ...
  const t = audioCopy(language);
  const [selected, select] = usePerformanceEditorState<string>("mixer", "selected", "");
  const [target, setTarget] = usePerformanceEditorState("mixer", "target", "");
  const [sourcePort, setSourcePort] = usePerformanceEditorState("mixer", "sourcePort", "");
  const [targetPort, setTargetPort] = usePerformanceEditorState("mixer", "targetPort", "");
  const [purpose, setPurpose] = usePerformanceEditorState<"main" | "send" | "custom">("mixer", "purpose", "main");
  const [stereo, setStereo] = usePerformanceEditorState("mixer", "stereo", true);
  const [processor, setProcessor] = usePerformanceEditorState("mixer", "processor", "");
  const [validation, setValidation] = usePerformanceEditorState<{ graph: AudioGraph; diagnostics: AudioDiagnostic[] } | null>("mixer", "validation", null);
  const diagnostics = validation?.graph === graph ? validation.diagnostics : [];
  ...
  const repairMaster = async () => {
    ...
    try {
      ...
      const fixed = await api.repairPerformanceMaster(buildSequencerConfigSnapshot(state.sequencer, bindings, graph, mixer));
      const current = useAppStore.getState();
      if (current.activeSessionState === "running" || current.audioGraph !== graph || current.mixer !== mixer || current.sequencerInstruments !== bindings || current.performanceWorkspaceGeneration !== state.performanceWorkspaceGeneration) return;
      const retained = new Set(fixed.instruments.map(b => b.id));
      useAppStore.setState({ audioGraph: fixed.audioGraph!, mixer: fixed.mixer!, sequencerInstruments: current.sequencerInstruments.filter(b => retained.has(b.id)) });
      setError(null);
    } catch (e) { setError(String(e)); }
  };
  const name = (id: string) => id === MASTER ? t("master") : id === "$output" ? t("audioOutput") : byId.get(id)?.name ?? `${t("missing")}: ${id}`;
  ...
  const check = async () => {
    try { setError(null); const result = await api.validateAudio(normalizeSessionInstrumentAssignments(bindings), graph, mixer); setValidation({ graph, diagnostics: result.diagnostics }); }
    catch (e) { setError(e instanceof Error ? e.message : String(e)); }
  };
  useEffect(() => {
    if (selected && !byId.has(selected)) { select(""); setSourcePort(""); }
    if (target && target !== "$output" && !byId.has(target)) { setTarget(""); setTargetPort(""); }
    if (processor && !patches.some(patch => patch.id === processor)) setProcessor("");
  }, [byId, patches, selected, target, processor, select, setSourcePort, setTarget, setTargetPort, setProcessor]);
  const add = () => {
  ...
  const throughMaster = async (id: string) => {
    try {
      const masterId = useAppStore.getState().ensureMaster();
      const current = useAppStore.getState(); const ports = mainPorts(byId.get(id), "output");
      const inputs = mainPorts(masterEndpoint, "input");
  ...
  const strip = (id: string) => {
    const state = mixer.strips[id] ?? defaultStrip(); const patch = byId.get(id); const master = id === MASTER;
    ...
    return <article key={id} className={`w-52 shrink-0 rounded-lg border p-3 ${master ? "sticky right-0 z-10 ml-auto border-cyan-600 bg-slate-900 shadow-[-8px_0_16px_#020617]" : selected === id ? "border-cyan-500 bg-slate-800" : "border-slate-700 bg-slate-900"}`}>
      <button className="w-full truncate text-left text-sm font-semibold" onClick={() => select(id)}>{name(id)}</button>
      ...
      <EditorDetails owner={`binding:${id}`} field="inserts" className="text-xs" summary={<>{t("inserts")} · {chain?.length ?? t("custom")}</>}>{() => <>{chain?.map((processorId, i) => <div key={processorId} className="my-1 flex items-center gap-1"><button className="truncate" onClick={() => open(processorId)}>{name(processorId)}</button><button disabled={running || !i} className={button} onClick={() => { const next = [...chain];[next[i - 1], next[i]] = [next[i], next[i - 1]]; applyGraph(wireInsertChain(graph, bindings, patches, id, next)); }}>↑</button><button disabled={running} className={button} onClick={() => { const audioGraph = wireInsertChain(graph, bindings, patches, id, chain.filter((p) => p !== processorId)); const strips = { ...mixer.strips }; delete strips[processorId]; useAppStore.setState({ audioGraph, sequencerInstruments: bindings.filter((b) => b.id !== processorId), mixer: { ...mixer, strips, sends: Object.fromEntries(Object.entries(mixer.sends).filter(([routeId]) => audioGraph.routes.some((r) => r.id === routeId))) } }); }}>×</button></div>)}</>}</EditorDetails>
      ...
      <EditorDetails owner={`binding:${id}`} field="additional" className="mt-2 text-[10px]" summary={<>{t("additional")}</>}>{() => <>{patch?.audio_interface?.groups.filter((g) => g.direction === "output").map((g) => <div key={g.id}>{g.name}: {g.ports.join(" / ")}</div>)}<div>{outputPorts(patch).join(" / ")}</div></>}</EditorDetails>
      {!master && <button className={`${button} mt-2`} onClick={() => { select(id); open(id); }}>{t("edit")}</button>}
  ...
  </div>{stereo && selected && target && <p className="mb-2 font-mono text-xs text-cyan-200">{t("mapping")}: {routingGroup(selected, "output")?.ports.join(" / ") ?? "?"} → {target === "$output" ? "left / right" : routingGroup(target, "input")?.ports.join(" / ") ?? "?"}</p>}<div className="flex flex-wrap gap-2"><select aria-label={t("inserts")} className={field} value={processor} disabled={running} onChange={(e) => setProcessor(e.target.value)}><option value="">{t("inserts")}…</option>{patches.filter((p) => p.always_on && p.audio_inlet_names.length && p.audio_outlet_names.length).map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}</select><button className={button} disabled={running || !selected || !processor} onClick={insert}>{t("add")}</button></div><EditorDetails owner={"mixer"} field="matrix" className="mt-3" summary={<>{t("matrix")}</>}>{() => <>{[...new Set([...endpoints.map((b) => b.id), ...graph.routes.map((r) => r.targetId)])].map((id) => ({ id })).map((b) => <div key={b.id} className="my-2 rounded border border-slate-800 p-2 text-xs"><button onClick={() => { setTarget(b.id); }}>{name(b.id)}</button>{groups.filter((rs) => rs[0].targetId === b.id).map((routes) => <EditorDetails owner={`route:${routes[0].id}`} field="matrix-channels" key={routes[0].id} className="ml-3" summary={<>{name(routes[0].sourceId)} → {name(b.id)} · {routes[0].kind} · {routes.length} ch <button className={button} disabled={running} onClick={() => removeRoutes(routes)}>{t("remove")}</button></>}>{() => <>{routes.map((r) => <div key={r.id} className="my-1 font-mono"><select aria-label={t("source")} disabled={running} className={field} value={r.sourcePort} onChange={(e) => applyGraph({ ...graph, routes: graph.routes.map((row) => row.id === r.id ? { ...row, sourcePort: e.target.value } : row) })}>{[...new Set([r.sourcePort, ...outputPorts(byId.get(r.sourceId))])].map((p) => <option key={p}>{p}</option>)}</select> → <select aria-label={t("destination")} disabled={running} className={field} value={r.targetId} onChange={(e) => applyGraph({ ...graph, routes: graph.routes.map((row) => row.id === r.id ? { ...row, targetId: e.target.value } : row) })}>{[...new Set([r.targetId, "$output", ...endpoints.map((b) => b.id)])].map((id) => <option key={id} value={id}>{name(id)}</option>)}</select> <select aria-label={t("mapping")} disabled={running} className={field} value={r.targetPort} onChange={(e) => applyGraph({ ...graph, routes: graph.routes.map((row) => row.id === r.id ? { ...row, targetPort: e.target.value } : row) })}>{[...new Set([r.targetPort, ...(b.id === "$output" ? ["left", "right"] : r.targetStage === "strip" ? outputPorts(byId.get(b.id)) : byId.get(b.id)?.audio_inlet_names ?? [])])].map((p) => <option key={p}>{p}</option>)}</select></div>)}</>}</EditorDetails>)}</div>)}</>}</EditorDetails><EditorDetails owner={"mixer"} field="diagram" className="mt-3" summary={<>{t("diagram")}</>}>{() => <><div className="flex flex-wrap gap-2 py-2">{groups.map((routes) => <div key={routes[0].id} className="rounded border border-slate-700 p-2 text-xs"><button onClick={() => select(routes[0].sourceId)}>{name(routes[0].sourceId)}</button> → <button onClick={() => { select(routes[0].targetId); setTarget(routes[0].targetId); }}>{name(routes[0].targetId)}</button><EditorDetails owner={`route:${routes[0].id}`} field="diagram-channels" summary={<>{t("mapping")}</>}>{() => <>{routes.map((r) => <div key={r.id}>{r.sourceStage}:{r.sourcePort} → {r.targetStage}:{r.targetPort} <button disabled={running} className={button} onClick={() => removeRoutes([r])}>×</button></div>)}</>}</EditorDetails></div>)}</div></>}</EditorDetails></>}</EditorDetails><EditorDetails owner={"mixer"} field="diagnostics" className="mt-3 border-t border-slate-800 pt-2" summary={<>{t("diagnostics")}</>}>{() => <><p className="my-2 text-xs text-slate-400">{t("structural")}</p><button className={button} onClick={() => void check()}>{t("validate")}</button>{[...audioGraphDiagnostics(bindings, patches, graph), ...diagnostics].map((d, i) => <button key={i} className="my-1 block text-left text-xs text-amber-200" onClick={() => { select(d.instanceId ?? ""); if (d.instanceId) open(d.instanceId); }}>{t(d.code in diagnosticKeys ? diagnosticKeys[d.code] : "diagnostics")}: {d.code in diagnosticKeys ? d.instanceId ? name(d.instanceId) : "" : d.message}</button>)}{Object.entries(mixer.strips).filter(([, s]) => s.mute || s.gainDb === null).map(([id]) => <button key={id} className="my-1 block text-xs text-amber-200" onClick={() => select(id)}>{name(id)}: {t("silent")}</button>)}</>}</EditorDetails> </>;
}

export function PerformMixer(props: { onStop: () => void }) {
  return <EditorScope><MixerShell {...props} /></EditorScope>;
}
function MixerShell(props: { onStop: () => void }) {
  const language = useAppStore(state => state.guiLanguage);
  return <EditorDetails owner="mixer" field="mixer" initiallyOpen className="mt-4 rounded-xl border border-slate-700 bg-slate-950/90 p-3 text-slate-200" summaryClassName="cursor-pointer text-sm font-semibold tracking-wide" summary={audioCopy(language)("mixer")}>{() => <MixerBody {...props} />}</EditorDetails>;
}
