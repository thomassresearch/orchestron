  ...
  return <section className={`rounded border p-2 ${selected ? "border-cyan-700" : "border-slate-700"}`}>
    ...
    {(selected || menu) && <div className="mt-2 space-y-2">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <button className={button} draggable onDragEnd={endArrangementDrag} onDragStart={e => { beginArrangementDrag(lane.id, item); e.dataTransfer.setData(ARRANGEMENT_ITEM_MIME, JSON.stringify({ trackId: lane.id, item })); }} onClick={() => insert([item], position, item.type === "pause")}>{c.add} {label(item)}</button>
      ...
      <div className="flex flex-wrap gap-1" aria-label={c.library}>{refs.map((ref, index) => <button key={index} className={button}
        ...
        }}>{ref.type === "pad" ? `P${ref.padIndex + 1}` : ref.type === "group" ? `${c.group} ${ref.groupId}` : ref.type === "super" ? `${c.super} ${ref.superGroupId}` : label(ref)}</button>)}</div>
