...
- Test instrument and performance data must live in versioned fixtures under a tests directory, independently of the dynamic developer-provided `examples/` directory. Tests must not read examples directly or through production template imports; frontend tests resolve the drumkit import to the shared `backend/tests/fixtures/patches/` snapshot.
